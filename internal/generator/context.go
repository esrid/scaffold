...
func buildStoreGenCtx(model *parser.Model, modulePath string, db string) storeGenCtx {
	...
	// INSERT: only new sqlite projects (db == "sqlite") use Go-side newID(); all others let the DB generate the UUID.
	var insertColsList []string
	var placeholders []string
	if db == "sqlite" {
		insertColsList = append(insertColsList, "id")
	...
	// Create args: newID() first only for new sqlite projects (db == "sqlite"); legacy sqlite and postgres let DB generate ID.
	var createArgParts []string
	if db == "sqlite" {
		createArgParts = append(createArgParts, "newID()")
	}
	for _, f := range fields {
