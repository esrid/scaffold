...
// placeholderAt returns the n-th (1-indexed) SQL placeholder for db: "$n"
// for postgres, "?" for sqlite.
func placeholderAt(db string, n int) string {
	if db == "postgres" {
...
func buildStoreGenCtx(model *parser.Model, modulePath string, db string) storeGenCtx {
	...
	// INSERT: id is generated in Go (uuid.NewV7()), passed explicitly like
	// any other column — no DB-side DEFAULT/extension needed.
	insertColsList := []string{"id"}
	placeholders := []string{placeholderAt(db, 1)}
	for i, f := range fields {
		insertColsList = append(insertColsList, f.Name)
		placeholders = append(placeholders, placeholderAt(db, i+2))
	}
	...
	for i, f := range fields {
		updateSetParts[i] = f.Name + " = " + placeholderAt(db, i+1)
	}
	updateSet := strings.Join(updateSetParts, ", ")

	// Create args: p.ID first (set by the store from uuid.NewV7() before
	// this query runs), then user fields.
	createArgParts := []string{"p.ID"}
	for _, f := range fields {
